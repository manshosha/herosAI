...
const OPTIONS = [
  {
    "value": "speaking_difficult",
    "label": "Speaking is difficult",
    "description": "Hard to express myself verbally"
  },
  {
    "value": "writing_reading_difficult",
    "label": "Writing or reading is hard",
    "description": "Struggle with written communication"
  },
  {
    "value": "communicate_well",
    "label": "I communicate well",
    "description": "No major issues"
  },
  {
    "value": "no_change",
    "label": "No change",
    "description": "Communication unaffected"
  }
];

export default function Q9CommunicationScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { strokeAnswers, updateStrokeAnswers } = useOnboardingState();

  const handleSelect = (value: string) => {
    updateStrokeAnswers({ communicationImpact: value as any });
  };

  const handleContinue = () => {
    router.push("/onboarding/q10-upper-extremity" as any);
  };
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      >
        ...
        <View style={styles.questionContainer}>
          <ThemedText type="title" style={styles.question}>
            How has the stroke affected your communication?
          </ThemedText>
          <ThemedText style={[styles.subtitle, { color: Colors.light.textSecondary }]}>
            We can include speech and language exercises
          </ThemedText>
        </View>
        ...
        <View style={styles.optionsContainer}>
          {OPTIONS.map((option) => (
            <GlassCard
              ...
              style={[
                styles.optionCard,
                strokeAnswers.communicationImpact === option.value && {
            ...
            >
              <View style={styles.optionContent}>
                ...
                <RadioButton label="" selected={strokeAnswers.communicationImpact === option.value} onPress={() => handleSelect(option.value)} />
              </View>
            </GlassCard>
          ))}
        </View>
      ...
      >
        <PrimaryButton label="Continue" onPress={handleContinue} disabled={!strokeAnswers.communicationImpact} />
      </View>
    </ThemedView>
  );
}
