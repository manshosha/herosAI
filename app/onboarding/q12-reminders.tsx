...
const OPTIONS = [
  {
    "value": "yes",
    "label": "Yes please",
    "description": "Help me stay consistent"
  },
  {
    "value": "maybe",
    "label": "Maybe try without first",
    "description": "See how I do first"
  },
  {
    "value": "no",
    "label": "No thanks",
...
export default function Q12RemindersScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { strokeAnswers, updateStrokeAnswers } = useOnboardingState();

  const handleSelect = (value: string) => {
    updateStrokeAnswers({ remindersPreference: value as any });
  };

  const handleContinue = () => {
    router.push("/onboarding/q13-medications" as any);
  };
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      >
        ...
        <View style={styles.questionContainer}>
          ...
          <ThemedText style={[styles.subtitle, { color: Colors.light.textSecondary }]}>
            We can send gentle nudges to keep you on track
          </ThemedText>
        </View>
        ...
        <View style={styles.optionsContainer}>
          {OPTIONS.map((option) => (
            <GlassCard
              ...
              style={[
                styles.optionCard,
                strokeAnswers.remindersPreference === option.value && {
            ...
            >
              <View style={styles.optionContent}>
                ...
                <RadioButton label="" selected={strokeAnswers.remindersPreference === option.value} onPress={() => handleSelect(option.value)} />
              </View>
            </GlassCard>
          ))}
        </View>
      ...
      >
        <PrimaryButton label="Continue" onPress={handleContinue} disabled={!strokeAnswers.remindersPreference} />
      </View>
    </ThemedView>
  );
}
